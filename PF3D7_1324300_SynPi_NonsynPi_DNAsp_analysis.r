...
Locus_nonsyn_syn_df <- read.csv("PF3D7_1324300_Pi_nonsyn_syn_estimates_by_country_rowwise_reduced.csv")
...
ggplot(Locus_nonsyn_syn_df_long, aes(x = Country, y = Value, fill = Type)) +
  ...
  labs(
    #title = "Nonsynonymous and Synonymous Nucleotide Diversity in PF3D7_1324300",
    x = "Country",
  ...
  theme(
    ...
    axis.title.y = element_text(size = 24, hjust = 1.1)
  ) +
  # Add region lines
...
mean(Locus_nonsyn_syn_df$Ratio_nonsyn_pi_syn_pi[is.finite(Locus_nonsyn_syn_df$Ratio_nonsyn_pi_syn_pi)], na.rm = TRUE)
