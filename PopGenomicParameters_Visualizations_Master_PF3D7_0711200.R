...
#add axis.text.x = element_text(angle = 45, hjust = 1, size = 12, margin = margin(t=10)), # Optional: tilt x-axis labels for readability
# in order to increase space between X-axis titles and bar graph 
...
ggplot(PF3D7_0711200_rowwise, aes(x = Geographic.Site, y = HaplotypeDiversity, fill = API_cat, pattern = Region)) + 
  ...
  scale_pattern_manual(values = c("Central Africa" = "wave", 
                                  ...
                                  "South East Asia" = "circle")) + # or any other pattern
  theme(
    #legend.position = "none", #include to remove legend (also comment out legend.text and legend.title)
    legend.text = element_text(size = 13), #legend text size
    legend.title = element_text(size = 15), #legend title size   
    plot.title = element_text(hjust = 0.5), #center the title 
...
ggplot(PF3D7_0711200_rowwise, aes(x = Geographic.Site, y = NeutralityIndex, color = API_cat_adjusted, shape = Region)) + 
  ...
  #ggtitle("Global Neutrality Index of Conserved Pf \nProtein, Unknown Function (PF3D7_0711200)") +  
...
ggplot(PF3D7_0711200_rowwise, aes(x = API_regional, y = Pi_pct, shape = Region, fill = API_cat_adjusted)) + 
  ...
  annotate("text", x = max(PF3D7_0711200_rowwise$API_regional * 0.9), y = max(PF3D7_0711200_rowwise$Pi_pct * 2), label = formula_text_API_adjusted, 
           hjust = 1, vjust = 0, color = "black", size = 7) +  # Add formula text  
  theme_minimal() +
  scale_y_continuous(breaks = seq(0, 0.65, by = .1), limits= c(0, 0.65)) +
  scale_fill_manual(values = c("High" = "deeppink3", "Moderate" = "darkorange", "Low" = "darkslategray1")) +  # Fill color for API_cat
  scale_shape_manual(values = c("Central Africa" = 21,  # Circle
...
##### Visualize Kruskal Wallis Ranks #####

# Calculate Nucleotide Diversity ranks manually
PF3D7_0711200_rowwise <- PF3D7_0711200_rowwise %>%
  mutate(rank = rank(Pi))

# Boxplot of ranks by group
library(ggplot2)
ggplot(PF3D7_0711200_rowwise, aes(x = API_cat_adjusted, y = rank)) +
  geom_boxplot() +
  labs(title = "Distribution of Ranks by Group",
       x = "Adjusted API Transmission Group",
       y = "Rank") +
  theme_minimal()

#Violin plot with specified factor levels
ggplot(PF3D7_0711200_rowwise, aes(x = API_cat_adjusted, y = rank)) +
  geom_violin(trim = FALSE, fill = "skyblue") +
  geom_boxplot(width = 0.1, outlier.shape = NA) +
  labs(title = "Density of Ranks by Group",
       x = "Group",
       y = "Rank") +
  theme_minimal()

# Strip plot with specified factor levels
ggplot(PF3D7_0711200_rowwise, aes(x = API_cat_adjusted, y = rank)) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  labs(title = "Individual Ranks by Group",
       x = "Group",
       y = "Rank") +
  theme_minimal()


# Calculate Haplotype Diversity ranks manually
PF3D7_0711200_rowwise <- PF3D7_0711200_rowwise %>%
  mutate(rank = rank(HaplotypeDiversity))
